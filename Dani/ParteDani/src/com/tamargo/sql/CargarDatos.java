...
public class CargarDatos {
     ...
     * @return conexion <- devuelve la variable conexión
     */
    public Connection realizarConexion(int bbdd) {
        ...
        switch (bbdd) {
            ...
            default -> conexion = conexiones.conexionSQlite();
        }
        return conexion;
    }
     ...
     * @return fecha <- la fecha bien construida
     */
     ...
     *
     * @param bbdd <- recibe un int que determinará a qué BBDD nos conectaremos
     * @return clientes <- devuelve un ArrayList con todas las visitas cargadas
     */
    public ArrayList<Visita> cargarVisitas(int bbdd) {
        ...
        try {
            ...
            while (r.next()) { // Recorremos los datos
                ...
                Date fecha;
                if (bbdd == 1)
                    fecha = reconstruirFechaSQLite(r.getString(6));
                else
                    fecha = r.getDate(6);
     ...
     *
     * @param bbdd <- recibe un int que determinará a qué BBDD nos conectaremos
     * @return clientes <- devuelve un ArrayList con todas las visitas organizadas por el guía
     */
    public ArrayList<Visita> cargarVisitasOrganizadasPorEmpleado(int bbdd, String dni) {
        ...
        try {
            ...
            if (visitas.size() <= 0)
                System.out.println("El empleado " + dni + " no ha participado ni se ha apuntado en ninguna visita aún");
            else
     ...
     *
     * @param bbdd <- recibe un int que determinará a qué BBDD nos conectaremos
     * @return clientes <- devuelve un ArrayList con todas las visitas de un cliente cargadas
     */
    public ArrayList<Visita> cargarVisitasDeUnCliente(int bbdd, String dni) {
        ...
        try {
            ...
            while (r.next()) { // Recorremos los datos
                ...
                while (r2.next()) {
                    Date fecha;
                    if (bbdd == 1)
                        fecha = reconstruirFechaSQLite(r2.getString(6));
                    else
                        fecha = r2.getDate(6);

                    visitas.add(new Visita(r2.getInt(1),
